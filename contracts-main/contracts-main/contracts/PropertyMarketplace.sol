...
contract PropertyMarketplace is Ownable, ERC2771Context {
	...
	event PropertyMetadataUpdated(string propertyId, string newPropertyURI);
	...
	/**
	 * @dev Updates a property's metadata URI
	 * @param _propertyId ID of the property to update
	 * @param _propertyURI New IPFS URI containing property details
	 */
	function updatePropertyMetadata(
		...
		// Update property URI
		properties[_propertyId].propertyURI = _propertyURI;

		// Emit event
		emit PropertyMetadataUpdated(_propertyId, _propertyURI);
	}
