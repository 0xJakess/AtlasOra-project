// Backend Integration Example - Complete Meta-Transaction System
...
const eip712Utils = require("./eip712-utils");
...
class BackendIntegrationExample {
	 ...
	 */
	async initialize() {
		console.log("🔧 Initializing Backend Integration Example...");
	...
	/**
	 * Example 5: Batch Meta-Transactions
	 */
	async example5_BatchMetaTransactions() {
		console.log("\n🎯 Example 5: Batch Meta-Transactions");
		console.log("====================================");
		
		console.log("📋 Creating multiple meta-transactions for batch processing...");
		
		const batchMetaTxs = [];
		const users = ["guest1", "guest2", "guest3"];
		const propertyIds = await this.contracts.propertyMarketplace.getAllPropertyIds();
		
		// Create multiple meta-transactions
		for (let i = 0; i < 3; i++) {
			const userKey = users[i];
			const user = this.dummyUsers[userKey];
			const mockSigner = this.createMockSigner(userKey);
			
			// Use different property for each booking
			const propertyId = propertyIds[i % propertyIds.length];
			
			const now = Math.floor(Date.now() / 1000);
			const checkInDate = now + 86400 * (30 + i * 7); // Spread out over weeks
			const checkOutDate = checkInDate + 86400 * 3; // 3 nights each
			...
			try {
				...
				batchMetaTxs.push({
					...
					propertyId,
					metaTx,
					checkInDate: new Date(checkInDate * 1000).toLocaleDateString(),
					checkOutDate: new Date(checkOutDate * 1000).toLocaleDateString()
				});
				
				console.log(`  ✅ Created meta-transaction for ${user.name}`);
				
			} catch (error) {
				console.log(`  ❌ Failed to create meta-transaction for ${user.name}: ${error.message}`);
			}
		}
		
		console.log(`\n📊 Batch Summary: ${batchMetaTxs.length} meta-transactions created`);
		
		// Execute batch (in real implementation, you might want to execute these in parallel)
		console.log("\n🚀 Executing batch meta-transactions...");
		
		for (let i = 0; i < batchMetaTxs.length; i++) {
			const batchItem = batchMetaTxs[i];
			console.log(`\n  Processing ${i + 1}/${batchMetaTxs.length}: ${batchItem.user}`);
	...
	/**
	 * Example 6: System Status and Analytics
	 */
	async example6_SystemAnalytics() {
		console.log("\n🎯 Example 6: System Status and Analytics");
		console.log("=========================================");
	...
	async runAllExamples() {
		console.log("🚀 Starting Backend Integration Examples");
		console.log("=======================================");
		...
		try {
			...
			await this.example5_BatchMetaTransactions();
			await this.example6_SystemAnalytics();
...
// Main execution
async function main() {
	const integrationExample = new BackendIntegrationExample();
	await integrationExample.runAllExamples();
}

// Export for use in other scripts
module.exports = BackendIntegrationExample;
